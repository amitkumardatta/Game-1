...
def setup(playername):
    ...
    def Score(hints, hintsused, chance, count, status):
        if hintsused == 0:
            h = 4
        elif hintsused == hints:
            h = 0
        elif hintsused >= hints / 2:
            h = 2
        elif hintsused < hints / 2:
            h = 3
        ...
        for i in range(1, chance + 1):
            ...
            if hints != 0:
                ...
                if choice != None:
                    ...
                    print("Continue Pressed, No Hints Used")
...
''')
    ...
    time.sleep(1.0)
    if hints <= hints2:
        print(f"*  You can use '{hints}' hint/s to guess number if you want")
    elif hints > hints2:
...
print(''' ~~ Instructions To Play "THE INTERACTIVE NUMBER GUESSING GAME" ~~
          ...
          $  If You are unable to guess the number in given chances 
